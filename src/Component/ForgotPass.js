...
const ForgotPass = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!validatePassword(newPassword)) {
      alert(
        "Please enter a valid password. It must contain at least one uppercase letter, one digit, and one special character."
      );
      return;
    }
    ...
    try {
      ...
      navigate(
        `/login?language=english&userType=${accountType}&userCategory=${userCategory}&instituteId=${getInstituteIdFromParam}`
      );
    } catch (error) {
